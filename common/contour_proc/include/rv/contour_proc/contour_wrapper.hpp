...
#include "rv/core/property_wrapper.hpp"
...
public:
    ...
    std::tuple<cv::Point2f, float> fittedCircle() const
    {
        if (!cache_flags.test(FITTED_CIRCLE_CALC))
        {
            ...
            {
                // 点数不足时触发异常
                RV_THROW_ERROR("Insufficient points for circle fitting");
            }
        }
    ...
    cv::RotatedRect fittedEllipse() const
    {
        if (!cache_flags.test(FITTED_ELLIPSE_CALC))
        {
            ...
            {
                // 点数不足时触发异常
                RV_THROW_ERROR("Insufficient points for ellipse fitting");
            }
        }
    ...
    static std::shared_ptr<ContourWrapper<ValueType>> getConvexHull(const std::vector<std::shared_ptr<ContourWrapper<ValueType>>> &contours)
    {
        ...
        for (const auto &contour : contours)
        {
            ...
            {
                RV_THROW_ERROR("Invalid contour");
            }
        }
...
public:
    ...
    const PointType &at(size_t idx) const noexcept
    {
        ...
        {
            RV_THROW_ERROR("Index out of range");
        }
